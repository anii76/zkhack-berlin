...
function quantizedToBytes32(value) {
...
const faceA1 = [
    ...
    .map(quantize);
const faceA1Bytes = faceA1.map(quantizedToBytes32);
const faceA2 = [
    -0.08908695727586746,
    0.10642766952514648,
    0.06451733410358429,
...
const faceA2Bytes = faceA1.map(quantizedToBytes32);
const faceB = [
    -0.09373191744089127,
    0.1104452982544899,
...
;
const faceBBytes = faceB.map(quantizedToBytes32);
// Set threshold for similarity (sum of squared differences)
const threshold = BigInt(300000000); // Adjust based on your similarity requirements
...
${thresholdStr}
`;
fs_1.default.writeFileSync("noir/zface_verifier/Prover.toml", proverToml);
let verifierContract;
before(() => __awaiter(void 0, void 0, void 0, function* () {
    verifierContract = yield hardhat_1.ethers.deployContract("HonkVerifier");
}));
it("has correct face embeddings", () => {
    (0, chai_1.expect)(faceA1).to.have.length(128);
    (0, chai_1.expect)(faceA2).to.have.length(128);
    (0, chai_1.expect)(faceB).to.have.length(128);
    ...
    (0, chai_1.expect)(Number(ssdA1A2)).to.be.lessThan(Number(threshold));
    (0, chai_1.expect)(Number(ssdA1B)).to.be.greaterThan(Number(threshold));
    (0, chai_1.expect)(Number(ssdA2B)).to.be.greaterThan(Number(threshold));
});
it("proves and verifies on-chain", () => __awaiter(void 0, void 0, void 0, function* () {
    // Deploy the ZFace contract with reference face and threshold
    const contractFactory = yield hardhat_1.ethers.getContractFactory("ZFace");
    const contract = yield contractFactory.deploy(yield verifierContract.getAddress(), faceA1Bytes, thresholdBytes);
    yield contract.waitForDeployment();
    // Generate a proof
    const { noir, backend } = yield hardhat_1.default.noir.getCircuit("zface_verifier");
    const input = {
    ...
    const { proof, publicInputs } = yield backend.generateProof(witness, {
        keccak: true,
    });
    console.debug({ proof });
    // Verify that public inputs match (reference face + threshold)
    (0, chai_1.expect)(publicInputs.length).to.eq(129);
    // Verify the proof on-chain
    const result = yield contract.verify(proof);
    (0, chai_1.expect)(result).to.eq(true);
    // You can also verify in JavaScript
    const resultJs = yield backend.verifyProof({
        proof,
        publicInputs,
    }, { keccak: true });
    (0, chai_1.expect)(resultJs).to.eq(true);
}));
it("fails verification with different faces", () => __awaiter(void 0, void 0, void 0, function* () {
